...
if (u.port !== '25061') {
  console.error(`ABORT: expected current port 25061, got ${u.port}`);
  await prisma.$disconnect();
  process.exit(1);
}
if (u.pathname !== '/generalbooks-pool') {
  console.error(`ABORT: expected current path /generalbooks-pool, got ${u.pathname}`);
  await prisma.$disconnect();
  process.exit(1);
}

u.port = '25060';
u.pathname = '/GeneralBooks_db';
